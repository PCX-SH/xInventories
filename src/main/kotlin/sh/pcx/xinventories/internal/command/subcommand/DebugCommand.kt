...
class DebugCommand : Subcommand {
    ...
    private suspend fun showInfo(plugin: PluginContext, sender: CommandSender) {
        ...
        sender.sendMessage("§7Version: §f${plugin.plugin.description.version}")
    ...
    private fun showPlayerInfo(plugin: PluginContext, sender: CommandSender, playerName: String?) {
    ...
    private suspend fun showStorageInfo(plugin: PluginContext, sender: CommandSender) {
        val storageService = plugin.serviceManager.storageService
        val config = plugin.configManager.mainConfig
